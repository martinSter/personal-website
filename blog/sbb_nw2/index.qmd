...
From the whole sequence of stations, we can see that there is a "Fahrt" with a stop in-between Näfels-Mollis and Ziegelbrücke, namely Nieder- und Oberurnen. So, `('Näfels-Mollis', 'Ziegelbrücke')` really seems to be a shortcut!
...
Are there any edges in `linien_edges` that were classified as shortcuts before?
...
A manual check of all 1'712 edges reveals that there are more shortcuts that our procedure was not able to identify. For example, the edge `(Bern, Zofingen)` cannot be identified because there is not other "Fahrt" that contains these two stations and stops somewhere in between. We manually remove such edges.
